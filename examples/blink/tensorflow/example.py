...
if __name__ == '__main__':    
    ...
    img_path = path.join(_DEELDATASET_BASE_DIR, "blink", "blink-3.0.1-20191120.zip_extracted", "final_db_anonymous", "noblink", "nb_8.bmp")
    pred_index = aModel.predict(img_path)
    print(label_names[pred_index])

    # Prediction overall image inside blink_right folder
    images = [f for f in glob.glob(path.join(_DEELDATASET_BASE_DIR, "blink", "blink-3.0.1-20191120.zip_extracted", "final_db_anonymous", "blink_right", "*.bmp"), recursive=True)]
